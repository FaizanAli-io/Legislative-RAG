...
class DocumentLoader:
    ...
    def normalize_text(self, text: str) -> str:
        ...
        # Fix common formatting issues in legislation
        text = re.sub(r'(\d+)\.\s*\n\s*(\w)', r'\1. \2', text)
        text = re.sub(r'([a-z])\n([a-z])', r'\1 \2', text)
    ...
    async def load_document(self, filepath_or_name: str | Path) -> Optional[Dict[str, Any]]:
        """Load and preprocess a single document by filename or relative path"""
        input_path = Path(filepath_or_name)

        # Case 1: Use directly if it exists
        if input_path.exists():
            file_path = input_path
        else:
            # Case 2: Resolve relative to `documents/`
            file_path = self.documents_dir / input_path
