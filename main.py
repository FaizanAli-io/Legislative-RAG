...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Launching RAG Backend")
...
async def process_documents(
    directory: Optional[str] = None, api_key: str = Depends(verify_api_key)
):
...
async def process_single_document(
    filename: str, api_key: str = Depends(verify_api_key)
):
...
async def query_documents(
    request: QueryRequest, api_key: str = Depends(verify_api_key)
):
...
async def search_documents(
    ...
    try:
        result = await rag_service.search_similar_chunks(
            ...
            similarity_threshold=request.similarity_threshold
            or settings.similarity_threshold,
        )
...
async def get_document_chunks(
    document_name: str, api_key: str = Depends(verify_api_key)
):
